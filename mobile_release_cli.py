...
def organize_platform_releases(file_path, platform):
    ...
    organized_data = defaultdict(lambda: defaultdict(lambda: {"Release Count": 0, "Versions Released": []}))

    for release in data:
        # For iOS Focus, include releases with "Focus / Klar" in their name
        if platform == "iOS" and 'focus' in file_path.lower() and "Focus" not in release['name']:
            continue
        ...
        organized_data[category][month]["Versions Released"].append(f"{release['name']} ({release['date']})")
...
def create_bar_chart(monthly_data):
    ...
    ax.set_xticks([x + bar_width * (n_categories / 2) - bar_width / 2 for x in range(n_months)])
    ...
    for platform, platform_data in combined_data.items():
        for release_type, releases in platform_data.items():
            for month, month_data in releases.items():
                monthly_data[month][f"{platform} - {release_type}"] += month_data["Release Count"]
...
def print_summary(combined_data, start_date, end_date):
    ...
    for platform, platform_data in combined_data.items():
        if platform != "Summary" and platform_data:  # Check if platform data is not empty
            print(f"  {platform}:\n")
            for release_type, releases in platform_data.items():
    ...
    print("***************************************", end="")  # 'end=""' to avoid extra newline
...
''',
    ...
    parser.add_argument('-repo', nargs='+', help='List of repositories to aggregate. Example: firefox-android firefox-ios focus-ios')
    parser.add_argument('-date_range', type=str, help='Date range in format MM/DD/YYYY-MM/DD/YYYY or Month-Month')
    parser.add_argument('--save', action='store_true', help='Save the output data to JSON files')
    parser.add_argument('--summary', action='store_true', help='Print a summary of the data')
    parser.add_argument('--graph', choices=['bar', 'plot'], help='Generate a graph (bar or plot)')

    return parser.parse_args()

def process_repositories(repos, temp_dir, start_date, end_date, github_token, combined_data):
    temp_file_paths = []
    for repo in repos:
        temp_file_name = os.path.join(temp_dir, f"{repo.replace('/', '_')}_releases.json")
        fetch_and_save_releases(repo, start_date, end_date, temp_file_name, github_token)
        ...
        if platform == "Android":
            ...
            for release_type, releases in organized_data["iOS"].items():
                for month, month_data in releases.items():
                    ...
                    combined_ios_data[release_type][month]["Release Count"] += month_data["Release Count"]
                    combined_ios_data[release_type][month]["Versions Released"].extend(month_data["Versions Released"])
    return temp_file_paths

def save_data_if_required(temp_file_paths, combined_data, save_flag):
    if save_flag:
        for temp_file_path in temp_file_paths:
            destination_file_path = os.path.join(os.getcwd(), os.path.basename(temp_file_path))
...
def main():
    ...
    combined_data = {
        "Android": {},
        "iOS": defaultdict(lambda: defaultdict(lambda: {"Release Count": 0, "Versions Released": []}))
    }

    with tempfile.TemporaryDirectory() as temp_dir:
        temp_file_paths = process_repositories(args.repo, temp_dir, start_date, end_date, github_token, combined_data)
        ...
        for platform, platform_data in combined_data.items():
            for release_type, releases in platform_data.items():
                for month, month_data in releases.items():
                    summary[platform][release_type] = {"Total Releases": summary[platform].get(release_type, {}).get("Total Releases", 0) + month_data["Release Count"]}
